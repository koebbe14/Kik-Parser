...
from PyQt5.QtGui import QBrush, QColor, QFont, QPixmap, QImage, QKeySequence, QTextDocument, QAbstractTextDocumentLayout
...
APP_VERSION = "3.32"  # Updated with delete keyword list feature
...
class BaseMessagesDialog(QDialog):
    ...
    def make_item(self, text, bg_color, tooltip="", user_role=None):
        # Replace NaN values with empty string to prevent "nan" from displaying
        if pd.isna(text):
            text = ''
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            if col == 0:  # Date
                ...
                return str(msg['sender']) if msg.get('sender') else ''
            elif col == 3:  # Receiver
                return str(msg['receiver']) if msg.get('receiver') else ''
            elif col == 4:  # Message
                return str(msg['message']) if msg.get('message') else ''
            elif col == 5:  # Tags
                return ', '.join(sorted(msg.get('tags', [])))
            elif col == 6:  # Media
                return str(msg.get('content_id', '')) or ' '
            elif col == 7:  # IP
                return str(msg.get('ip', ''))
            elif col == 8:  # Port
                return str(msg.get('port', ''))
            elif col == 9:  # Source
                return os.path.basename(msg.get('source', 'Unknown'))
            elif col == 10:  # Line Number
                return str(msg.get('line_number', ''))
...
class KikAnalyzerGUI(QMainWindow):
    ...
    def apply_theme(self):
        ...
        if hasattr(self, 'message_table') and hasattr(self, 'search_timer'):
    ...
    def show_help(self):
        ...
        help_text_edit.setHtml(
            ...
            "</ul>"
            "<h3>Color Legend for Row Backgrounds</h3>"
            "<ul>"
            "<li><span style='background-color: #ff0000; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Bold Red&nbsp;</span>: Tagged as <b>CSAM</b> (highest priority).</li>"
            "<li><span style='background-color: #ff8000; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Bold Orange&nbsp;</span>: Tagged as <b>Evidence</b>.</li>"
            "<li><span style='background-color: #ffff00; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Bold Yellow&nbsp;</span>: Tagged as <b>Of Interest</b>.</li>"
            "<li><span style='background-color: #9fe780; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Green&nbsp;</span>: Message matches selected keyword list.</li>"
            "<li><span style='background-color: #f3f6f4; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Very Light Gray&nbsp;</span>: Message from <b>Sender 1</b>.</li>"
            "<li><span style='background-color: #9fc5e8; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Light Blue&nbsp;</span>: Message from <b>Sender 2</b>.</li>"
            "<li><span style='background-color: #e0ffe0; color: black; padding: 2px 6px; border-radius: 3px;'>&nbsp;Light Green&nbsp;</span>: Message tagged with a <b>Custom Tag</b>.</li>"
            "</ul>"
            ...
            "</ul>"
            "<p><b>Version:</b> 3.32 | <b>GitHub:</b> <a href='https://github.com/Koebbe14/Kik-Parser'>Koebbe14/Kik-Parser</a></p>"
        )
    ...
    def show_context_menu(self, position):
        ...
        # For any other column, show tag menu
        selected_rows = sorted(set(idx.row() for idx in self.message_table.selectedIndexes()))
        if not selected_rows:
            # If no selection, select the row that was right-clicked
        ...
        try:
            ...
            if export_format == "HTML":
                ...
                with open(output_file, 'w', encoding='utf-8') as f:
                    ...
                    f.write("""
<style>
body { font-family: Arial, sans-serif; margin: 20px; background-color: #f4f4f9; transition: background-color 0.3s, color 0.3s; }
h2 { color: #333; }
.controls { margin-bottom: 20px; }
select { padding: 8px; font-size: 16px; border-radius: 4px; }
table { border-collapse: collapse; width: 100%; background-color: #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.1); table-layout: auto; }
th, td { border: 1px solid #ddd; padding: 12px; text-align: left; word-wrap: break-word; max-width: 300px; }
th { background-color: #4CAF50; color: white; cursor: pointer; position: sticky; top: 0; z-index: 2; }
th.sort-asc::after { content: ' ↑'; }
th.sort-desc::after { content: ' ↓'; }
tr:nth-child(even) { background-color: #f5f5f5; }
tr:hover { background-color: #f1f1f1; }
tr.tag-csam { background-color: #ff0000 !important; color: #fff; }
tr.tag-evidence { background-color: #ff8000 !important; color: #000; }
tr.tag-interest { background-color: #ffff00 !important; color: #000; }
tr.tag-custom { background-color: #e0ffe0; }
tr.keyword-hit { background-color: #9fe780; }
tr.sender1 { background-color: #f3f6f4; }
tr.sender2 { background-color: #9fc5e8; }
...
body.dark-mode table { background-color: #444; }
body.dark-mode th { background-color: #2E7D32; }
body.dark-mode tr:nth-child(even) { background-color: #555; }
body.dark-mode tr:hover { background-color: #666; }
body.dark-mode tr.tag-csam { background-color: #cc0000 !important; }
body.dark-mode tr.tag-evidence { background-color: #cc6600 !important; }
body.dark-mode tr.tag-interest { background-color: #cccc00 !important; }
body.dark-mode tr.tag-custom { background-color: #b3ccb3; }
body.dark-mode tr.keyword-hit { background-color: #80c660; }
body.dark-mode tr.sender1 { background-color: #c3c6c4; }
body.dark-mode tr.sender2 { background-color: #80a5c8; }
...
.legend { margin: 10px 0; padding: 10px; background-color: #f9f9f9; border-radius: 5px; }
.legend-item { padding: 5px 10px; margin: 2px; border-radius: 3px; display: inline-block; font-weight: bold; }
body.dark-mode .legend { background-color: #444; }
</style>
<script>
...
<div class="legend">
<h3>Color Legend</h3>
<span class="legend-item" style="background-color: #ff0000; color: white;">CSAM</span>
<span class="legend-item" style="background-color: #ff8000; color: black;">Evidence</span>
<span class="legend-item" style="background-color: #ffff00; color: black;">Of Interest</span>
<span class="legend-item" style="background-color: #e0ffe0; color: black;">Custom Tag</span>
<span class="legend-item" style="background-color: #9fe780; color: black;">Keyword Hit</span>
<span class="legend-item" style="background-color: #f3f6f4; color: black;">Sender 1</span>
<span class="legend-item" style="background-color: #9fc5e8; color: black;">Sender 2</span>
</div>
""")
                    ...
                    for msg, conv_id in messages_to_export:
                        ...
                        for i, field in enumerate(selected_fields):
                            if field == 'conversation':
                                ...
                                sort_val = pri
                                f.write(f"<td data-sort-value=\"{sort_val}\">{value}</td>\n")
                                continue
                            ...
                            f.write(
                                ...
                                f"title='{html.escape(str(raw_val)) if field in ['sender', 'receiver', 'message'] else ''}'>"
                                f"{value}</td>\n"
                            )
    ...
    def load_data(self):
        try:
            ...
            # 3. Create the main text QLabel using the original content
            text_label = QLabel(
            ...
            # Apply theme stylesheet
            if hasattr(self, 'theme_manager'):
                folder_msg.setStyleSheet(self.theme_manager.get_dialog_stylesheet())
            else:
                folder_msg.setStyleSheet("""
                    QMessageBox { background-color: #f4f4f9; }
                    QLabel { font-size: 16px; }
                    QPushButton {
                        font-size: 14px;
                        padding: 8px;
    ...
    def populate_conversations(self, df):
        ...
        for row in df.itertuples():
            ...
            line_number = int(line_num_val) if pd.notna(line_num_val) else 0
    ...
    def load_progress(self):
        ...
        try:
            ...
            self.status_bar.showMessage(
                ...
                f"{loaded_tags} tagged messages, {loaded_notes} notes"
            )
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Information)
